...
const SegmentBlock = ({
  ...
  const cabin = segment.departure.cabin?.replaceAll('_', ' ');
    ...
    <article className="tv-itin-segment">
      ...
      <div className="tv-itin-body">
        <div className="tv-itin-when">
          ...
          {nextDayWarning ? <span className="tv-itin-alert">Departs the next day</span> : null}
        </div>
        ...
        <div className="tv-itin-flight">
          ...
          <span className="tv-itin-sub">{[aircraft, flightCode, cabin].filter(Boolean).join(' • ')}</span>
        </div>
        <div className="tv-itin-when">
          <span className="tv-itin-line">
            <MdOutlineCalendarMonth className="tv-itin-ico" aria-hidden />
  ...
  return (
    <div className="tv-itin-layover">
      ...
      <div className="tv-itin-layover-copy">
        <span>{label ? `${label} layover at ${airportName}` : `Layover at ${airportName}`}</span>
...
export const FlightItinerary = ({ title, leg }: { title: string; leg: FlightLeg }): JSX.Element => {
  const segments =
    (leg.segments?.length ?? 0) > 0
      ? leg.segments
      : [
          {
    ...
    <section className="tv-timeline">
      <div className="tv-timeline-title">
        ...
        <span className="muted">
          {stopsLabel(leg.numberOfStops)} • {formatDuration(leg.duration)} • {leg.flightNumber}
        </span>
      </div>
